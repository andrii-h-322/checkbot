...
class AIVerifier:
        ...
        content_items = [
            {
                ...
                "text": (
                    ...
                    f"Критерии проверки:\n{criteria}\n\n"
                    "Проанализируй изображения. Убедись, что они соответствуют критериям (скриншоты из TikTok, виден комментарий с текстом 'Мне скинул @fulyashki' или похожим).\n"
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "Ты профессиональный верификатор скриншотов для выдачи доступа в Telegram-канал. "
                    "Твоя задача — строго проверить скриншоты на соответствие критериям: это должны быть скриншоты из TikTok, "
                    "на которых оставлены комментарии с текстом 'Мне скинул @fulyashki' (или подобным текстом). "
    ...
    async def _verify_with_gemini(
        ...
        prompt = (
            ...
            f"Тебе предоставлено {len(images_bytes)} скриншот(ов).\n"
            f"Критерии проверки:\n{criteria}\n\n"
            "Внимательно изучи предоставленные скриншоты:\n"
            "• Проверь, что на скриншотах виден интерфейс TikTok и комментарии пользователя с текстом 'Мне скинул @fulyashki' (или похожим).\n"
            "• Если все условия выполнены — установи approved=true и подтверди выполнение.\n"
            "• Если скриншоты не из TikTok, комментарии отсутствуют, не содержат нужного текста, обрезаны или неразборчивы — "
        ...
        content_items = [
            {
                ...
                "text": (
                    f"Тебе предоставлено {len(images_bytes)} скриншот(ов).\n"
                    f"Критерии проверки:\n{criteria}\n\n"
                    "Проанализируй изображения. Убедись, что они соответствуют всем критериям (скриншоты из TikTok, виден комментарий с текстом 'Мне скинул @fulyashki' или похожим).\n"
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "Ты профессиональный верификатор скриншотов для Telegram-бота. "
                    "Твоя задача — проверять скриншоты из TikTok на оставленные комментарии 'Мне скинул @fulyashki' (или похожие). "
                    "Всегда возвращай валидный JSON: {\"approved\": true/false, \"reason\": \"текст на русском\"}."
                )
            },
            {
                "role": "user",
