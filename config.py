...
class Settings(BaseSettings):
    ...
    # Критерии для проверки AI (демонстрационный пример для проверки работоспособности)
    verification_criteria: str = Field(
        default=(
            "Проверь предоставленный скриншот: этот бот является примером для проверки работоспособности AI-верификации. "
            "1) Проверь, виден ли на скриншоте интерфейс TikTok с оставленным комментарием пользователя (или подтверждение выполнения тестового задания); "
            "2) Если на скриншоте видно выполнение условия — одобри заявку (approved=true); "
            "3) Если скриншот не относится к заданию, неразборчив или условие не выполнено — отклони (approved=false) и вежливо объясни причину на русском языке."
        ),
